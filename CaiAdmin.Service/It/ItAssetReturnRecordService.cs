...
namespace ItSys.Service
{
    public class ItAssetReturnRecordService
        : BaseService<ItAssetUseRecord>
    {
        public ItAssetReturnRecordService(Repository<ItAssetUseRecord> repository)
             : base(repository)
        {
        ...
        private ISugarQueryable<ItAssetUseRecord> BuildWhere(ISugarQueryable<ItAssetUseRecord> query, ItAssetReturnRecordQueryDto queryDto)
        {
            query = query.Where(e => e.record_type == "RETURN");
            ...
            #endregion
            #region 交还地点
            if (!string.IsNullOrWhiteSpace(queryDto.place))
            {
                query = query.Where(e => e.place.Contains(queryDto.place));
            }
            #endregion
            #region 交还备注
            ...
            #endregion
            #region 交还日期
            if (queryDto.return_date_begin.HasValue)
            {
                query = query.Where(e => e.record_date >= queryDto.return_date_begin);
            }
            if (queryDto.return_date_end.HasValue)
            {
                query = query.Where(e => e.record_date < queryDto.return_date_end.Value.AddDays(1));
            }
            #endregion
            return query;
        }
         
        public async Task<PageListDto<ItAssetReturnRecordDto>> GetPageListAsync(ItAssetReturnRecordQueryDto queryDto)
        {
            var query  =this.Repository.AsQueryable();
            query = this.BuildWhere(query, queryDto);
            var result = new PageListDto<ItAssetReturnRecordDto>();
            result.Total = await query.Clone().CountAsync();
            result.List = await query.Select(e => new ItAssetReturnRecordDto { }).BuildPageByQuery(queryDto).BuildOrderByQuery(queryDto).ToListAsync();
            return result;
        }
