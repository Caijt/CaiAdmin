...
{
    public class RoleServiceBak : EntityService<RoleSaveDto, RoleSaveDto, RoleDto, RoleQueryDto, Role, int>
    {
        public RoleServiceBak(AppDbContext appDbContext, IMapper mapper, IEnumerable<IEntityHandler<Role>> entityHandlers, IEnumerable<IQueryFilter<Role>> queryFilters) : base(appDbContext, mapper, entityHandlers, queryFilters)
        {
        ...
        /// <returns></returns>
        public async Task<List<MenuPermissionDto>> GetRoleMenusAsync(int roleId)
        {
            List<MenuPermissionDto> permissionMenus;
            if (roleId > 0)
            {
                var menus = await DbContext.Set<RoleMenu>().Where(e => e.RoleId == roleId).ToListAsync();
                var menuIds = menus.Select(e => e.MenuId);
                var menuWithParentIds = await DbContext.Set<Menu>().Where(e => menuIds.Contains(e.Id)).Select(e => (e.ChainIds == null ? "" : e.ChainIds + ",") + e.Id.ToString()).ToListAsync();

                if (menuWithParentIds.Count > 0)
                {
                    var menuAllIds = string.Join(",", menuWithParentIds).Split(",").Distinct().Select(e => int.Parse(e));
                    permissionMenus = await DbContext.Set<Menu>().Where(e => menuAllIds.Contains(e.Id)).OrderBy(e => e.Order).Select(e => new MenuPermissionDto
                    {
                        //Id = e.Id,
                        //Name = e.Name,
                        //ParentId = e.ParentId,
                ...
                {
                    permissionMenus = new List<MenuPermissionDto>();
                }
            }
            else
            {
                permissionMenus = await DbContext.Set<Menu>().OrderBy(e => e.Order).Select(e => new MenuPermissionDto
                {
                    //Id = e.Id,
                    //Name = e.Name,
                    //ParentId = e.ParentId,
